...
begin
  ...
  if not cbNaoLimparDadosGravados.Checked then
    dmDados.ConexaoAgilus.Execute('truncate table tmp_siape_contratos_excluidos');
  ...
  begin
    ...
    begin
      ...
      if UpperCase(RightStr(nome, 9)) = ' PENS EXC' then
        NomeBanco := ReplaceStr(nome, ' PENS EXC', '');
      if UpperCase(RightStr(nome, 9)) = ' SERV EXC' then
        NomeBanco := ReplaceStr(nome, ' SERV EXC', '');
      ...
      try
        repeat
          ...
          begin
            SqlInsert := SqlInsert +
              'insert into tmp_siape_contratos_excluidos(TIPO_FUNCIONAL, UF, ORGAO, UPAG, MATRICULA, MATRICULA_INSTITUIDOR, ' +
              ...
              QuotedStr(Copy(linha, 168, 12))+', '+                        // SITUACAO_FUNCIONAL
              QuotedStr(Copy(linha, 180, 20))+', '+                        // NUMERO_CONTRATO
          ...
          begin
            SqlInsert := SqlInsert +
              ...
              QuotedStr(NomeBanco)+', '+                                   // BANCO
              QuotedStr(Copy(linha, 165, 12))+', '+                        // SITUACAO_FUNCIONAL
              QuotedStr(Copy(linha, 177, 20))+', '+                        // NUMERO_CONTRATO
